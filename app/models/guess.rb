class Guess < ActiveRecord::Base

  validates :game_id, :round_num, :site_id, :lat_true, :long_true, presence: true

  belongs_to :game, :inverse_of => :guesses

  belongs_to :unesco_site,
    class_name: :UnescoSite,
    foreign_key: :site_id,
    primary_key: :id_no

  def self.return_5_guesses(sites)
    sites.map{ |site| Guess.new(site) }
  end

  def update_points!(lat_guess, long_guess)
    distance = Guess.haversine_distance([lat_guess.to_f, long_guess.to_f],[self.lat_true, self.long_true])
    points = Guess.points(distance)
    self.points = [points,2].max
    self.save!
  end

  def self.haversine_distance(loc1, loc2)
    rad_per_deg = Math::PI/180
    rkm = 6371

    dlat_rad = (loc2[0]-loc1[0]) * rad_per_deg
    dlon_rad = (loc2[1]-loc1[1]) * rad_per_deg

    lat1_rad, lon1_rad = loc1.map {|i| i * rad_per_deg }
    lat2_rad, lon2_rad = loc2.map {|i| i * rad_per_deg }

    a = Math.sin(dlat_rad/2)**2 + Math.cos(lat1_rad) * Math.cos(lat2_rad) * Math.sin(dlon_rad/2)**2
    c = 2 * Math::atan2(Math::sqrt(a), Math::sqrt(1-a))

    return rkm * c
  end

  def self.points(distance)
    4000*(2.71828**(distance*(-0.0002)))
  end

end
